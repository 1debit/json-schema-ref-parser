...
$RefParser.prototype.dereference = async function dereference (path, schema, options, callback) {
  let me = this;
  let args = normalizeArgs(arguments);
  let collectCircularPaths = args.options.dereference.circular === 'collect'
  if (collectCircularPaths) {
    args.options.circularPaths = {}
  }
  ...
  try {
    ...
    finalize(me);
    if (collectCircularPaths) {
      return maybe(args.callback, Promise.resolve(args.options.circularPaths));
    } else {
      return maybe(args.callback, Promise.resolve(me.schema));
    }
  }
